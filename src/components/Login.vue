...
>
  ...
  <body>
    ...
    <div class="login">
        <h1>WELCOME TO EXPERIENCE EVENTS</h1>
        ...
        login()
        {
            axios.post(
               // `http://127.0.0.1:8000/api/login?email=${this.email}&password=${this.password}`
                process.env.VUE_APP_API_URL+'login', {email:this.email, password:this.password}

            ).then((res) => {
                console.log(res.data);
                localStorage.setItem("user-info", res.data.data.token);
